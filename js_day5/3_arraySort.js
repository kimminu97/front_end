...
points.sort() //3.정수값 배열- 사전식 알파벳순서[1, 10, 100, 25, 40, 5] 
                // ->sort에 콜백함수로 정렬방법(값의 크기) 전달
console.log(points)
points.sort(function(a,b){  //4. sort 메소드의 callback함수사용

    return a-b; //a>b일때는 양수, a<b일때는 음수, a==b일때는 0 리턴 -> 이 결과로 정렬
})
console.log(points)     //위의 콜백함수를 적용하여 정렬한 결과 //[1, 5, 10, 25, 40, 100]
points.reverse();       //5.point.sort에서 한번 적용된 콜백함수 정렬기준이 계속 적용
console.log('**'+points)
//값이 큰 순서에서 작은 순서로
points.reverse(function(a,b){   //6. points 배열을 처음 정렬한다면 역순은 이렇게 합니다.
    return a-b; //a>b일때는 양수, a<b일때는 음수, a==b일때는 0 리턴 -> 어떤 값이 더큰지 알아내려면 뺄셈
})
console.log(points)
...
//{type:'BMW',year:2010}는 객체, 객체를 미리 클래스로 정의하지 않고 사용할때 정의(프로토타입)
